    ...
    for reception in receptions:
        if hasattr(reception,'payment'):
            if hasattr(reception.payment,'paymentrecord_set'):
                payment_set = PaymentRecord.objects.filter(payment_id = reception.payment.id, status='paid')
                try:
...
def pick_up_excel(request):
    ...
    #엑셀 파일 불러오기
    wb = load_workbook('static/excel_form/Pick_up_list.xlsx') #Workbook()
...
def reservation_info(request):
    ...
    print('reservation_reception_id: ', reservation_reception_id)
    context = {
        ...
        'need_invoice':'' if reception is None else reception.need_invoice,   
        'need_insurance':'' if reception is None else reception.need_insurance, 
        ...
        'funnel': reservation.funnel if reservation.patient is None else reservation.patient.funnel,
...
def document_excel(request, reception_id):
    ...
    response = HttpResponse(content_type='application/ms-excel')
    response['Content-Disposition'] = 'attachment; filename="Document_report_' + str(reception_id) + '.xlsx"'
    diagnosis = True
    ...
    for reception in receptions:
        ...
        wb = load_workbook('static/excel_form/Document_report.xlsx') #Workbook()
        border_thin = Border(top=Side(border_style="thin", color="000000") ,
                            left=Side(border_style="thin", color="000000") ,
                        right=Side(border_style="thin", color="000000") ,
                        bottom=Side(border_style="thin", color="000000") )
        ...
        if medicine_receipt == True:
            ...
            ws['B' + str(38)] = 'Tổng trước thuế/Sub Total without VAT'
            ws['B' + str(38)].font = Font(bold = True)
            ws['G' + str(38)] = f"{sub_total:,}"

            ws['B' + str(39)] = 'Thuế GTGT/VAT ( %)'
            ws['B' + str(39)].font = Font(bold = True)
            ws['G' + str(39)] = vat

            ws['B' + str(40)] = 'Tổng tiền/Total'
            ws['B' + str(40)].font = Font(bold = True)
            ws['G' + str(40)] = f"{int(total):,}"
