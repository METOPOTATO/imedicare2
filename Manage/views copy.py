    ...
    for roof_cnt in range(len(argument_list) // 500  + 1):
        ...
        for i in range(receptions.count()):
            ...
            for tmp_medicine in tmp_medicine_set:
                ...
                ws['I' + str(temp_row)] = tmp_medicine.amount * tmp_medicine.days * tmp_medicine.medicine.get_price(recorded_date)
                ws['J' + str(temp_row)] = "Medicine"
                temp_row += 1
                count += 1
            ...
            for tmp_test in tmp_test_set:
                ...
                ws['H' + str(temp_row)] = "-"
                ws['I' + str(temp_row)] = tmp_test.test.get_price(recorded_date)
                ws['J' + str(temp_row)] = "Lab"
                temp_row += 1
                count += 1
            ...
            for tmp_precedure in tmp_precedure_set:
                if 'R' in tmp_precedure.precedure.code:
                    ...
                    ws['I' + str(temp_row)] = tmp_precedure.precedure.get_price(recorded_date)
                    ws['J' + str(temp_row)] = "Radiation"
                    temp_row += 1
                    count += 1
            ...
            ws['K' + str(current_row)] = receptions[i].payment.sub_total
            if receptions[i].payment.discounted != 0:
                ws['L' + str(current_row)] = receptions[i].payment.sub_total / 100 * receptions[i].payment.discounted
            else:
                ws['L' + str(current_row)] = receptions[i].payment.discounted_amount
            ws['M' + str(current_row)] = receptions[i].payment.additional
            ws['N' + str(current_row)] = receptions[i].payment.total
            ws['O' + str(current_row)] = receptions[i].payment.total - paid_sum
            ws['P' + str(current_row)] = paid_sum

            payment_record = receptions[i].payment.paymentrecord_set.filter(status = 'paid')

            if payment_record:
                ws['Q' + str(current_row)] = receptions[i].payment.paymentrecord_set.first().method
            else:
                ws['Q' + str(current_row)] = '-'
                
            if payment_record:
                str_record = ''
                for record_data in payment_record:
                    str_record += record_data.memo + "\n"
                ws['R' + str(current_row)] = str_record
            ...
            if count != 0:
                ...
                ws.merge_cells('K' + str(current_row) + ':K' + str(current_row + count-1))
...
def rec_report_excel(request):
    ...
    for reception in receptions:
       ...
       #ws['V' + str(current_row)].number_format  = number_format
        ...
        ws['W' + str(current_row)] = reception.payment.progress
        ws['X' + str(current_row)] = reception.doctor.name_eng
        #if paymentrecords:
        ...
        if paymentrecords:
            ...
            for record_data in paymentrecords.order_by("-date"):
                str_record += record_data.memo + "\n"
            ws['Y' + str(current_row)] = str_record
...
def cumstomer_management_excel(request):
    ...
    #엑셀 파일 불러오기
    wb = load_workbook('static/excel_form/customer_excel_form.xlsx') #Workbook()
    ws = wb.active# grab the active worksheet
...
def debit_report_excel(request):
    ...
    #엑셀 파일 불러오기
    wb = load_workbook('static/excel_form/EX THEO DOI CONG NO.xlsx') #Workbook()
    ws = wb.active# grab the active worksheet
...
def test_statistics_excel(request):
    ...
    #이름 설정
    response = HttpResponse(content_type='application/ms-excel')
    response['Content-Disposition'] = 'attachment; filename="EX STATISTICS_' + start_date + '_' + end_date +'.xlsx"'
    

    #엑셀 파일 불러오기
    wb = load_workbook('static/excel_form/EX STATISTICS.xlsx') #Workbook()

    ws = wb.get_sheet_by_name('Test statistics')# grab the active worksheet
    ...
    ##Procedure
    ws = wb.get_sheet_by_name('Procedure statistics')

    ws['E3'] = 'Ngày/ From: : ' + start_date 
    ws['F3'] = 'đến ngày/to : ' + end_date
    ...
    for procedure in procedures:
        ...
        for depart in depart_list:
            ...
            ws['A' + str(current_row)] = writing_number
            ws['B' + str(current_row)] = depart['name']
            ws['C' + str(current_row)] = procedure.code 
            ws['D' + str(current_row)] = procedure_class_dict[str(procedure.precedure_class_id)]
            ws['E' + str(current_row)] = procedure.name
            ws['F' + str(current_row)] = sub_query_depart.count()
            ws['G' + str(current_row)] = procedure.get_price()
            ws['H' + str(current_row)] = '=+F' + str(current_row) + '*G' + str(current_row)
            ws['G' + str(current_row)].number_format = '_-* #,##0 _₫_-;-* #,##0 _₫_-;_-* "-"?? _₫_-;_-@_-'
            ws['H' + str(current_row)].number_format = '_-* #,##0 _₫_-;-* #,##0 _₫_-;_-* "-"?? _₫_-;_-@_-'
            ...
            writing_number +=1 
            current_row +=1
    rows = ws['A8:H' + str(current_row + 3)]
    for row in rows:
        for cell in row:
            cell.border = border_thin
            cell.font = font_base

    ws['E' + str(current_row + 3)] = 'Tổng/ Total'
    ws['E' + str(current_row + 3)].font = Font(bold = True)
    ws['F' + str(current_row + 3)] = '=+SUM(F8:F' + str(current_row) + ')'
    ws['F' + str(current_row + 3)].font = Font(bold = True)
    ws['H' + str(current_row + 3)] = '=+SUM(H8:H' + str(current_row) + ')'
    ws['H' + str(current_row + 3)].number_format = '_-* #,##0 _₫_-;-* #,##0 _₫_-;_-* "-"?? _₫_-;_-@_-'
    ws['H' + str(current_row + 3)].font = Font(bold = True)

    ##Package
    ws = wb.get_sheet_by_name('Package statistics')
    ...
    package_class_query = PrecedureClass.objects.all()
    package_class_dict = {}
    package_code_dict = {}
    for package_class in package_class_query:
        package_class_dict[str(package_class.id)] = package_class.name
        package_code_dict[str(package_class.id)] = package_class.code
    ...
    for package in packages:
        ...
        # sub_query = Reception.objects.filter(
        #     recorded_date__range = (date_min, date_max), 
        #     diagnosis__preceduremanager__precedure = package.id,
        # ).exclude(
        #     progress='deleted'
        # ).prefetch_related(
        #     'diagnosis__preceduremanager_set',
        ...
        sub_query = Package_Manage.objects.filter(
                date_bought__range = (date_min, date_max),
                itme_round = '1').prefetch_related('precedure').filter(precedure__code=package.code).order_by('date_bought', 'patient_id', 'depart' ) 
        if not sub_query:
            continue
        for depart in depart_list:
            sub_query_depart = sub_query.filter(depart = depart['id'])
            if sub_query_depart.count() == 0:
                continue

            ws['A' + str(current_row)] = writing_number
            ws['B' + str(current_row)] = depart['name']
            ws['C' + str(current_row)] = package.code 
            ws['D' + str(current_row)] = package_class_dict[str(package.precedure_class_id)]
            ws['E' + str(current_row)] = package.name
            ws['F' + str(current_row)] = sub_query_depart.count()
            ws['G' + str(current_row)] = package.get_price()
            ws['H' + str(current_row)] = '=+F' + str(current_row) + '*G' + str(current_row)
            ws['G' + str(current_row)].number_format = '_-* #,##0 _₫_-;-* #,##0 _₫_-;_-* "-"?? _₫_-;_-@_-'
            ws['H' + str(current_row)].number_format = '_-* #,##0 _₫_-;-* #,##0 _₫_-;_-* "-"?? _₫_-;_-@_-'
...
def KBL_project_excel(request):
    ...
    #엑셀 파일 불러오기
    wb = load_workbook('static/excel_form/Project_excel.xlsx') #Workbook()
    ws = wb.active# grab the active worksheet
...
def PM_SPECIAL1_excel(request):
    ...
    #엑셀 파일 불러오기
    wb = load_workbook('static/excel_form/PM_statistics_form.xlsx') #Workbook()
    ws = wb.active# grab the active worksheet
...
def exam_inventory_excel(request):
    ...
    #엑셀 파일 불러오기
    wb = load_workbook('static/excel_form/ExamFee_list.xlsx') #Workbook()
    ws = wb.active# grab the active worksheet
...
def test_inventory_excel(request):
    ...
    #엑셀 파일 불러오기
    wb = load_workbook('static/excel_form/Test_list.xlsx') #Workbook()
    ws = wb.active# grab the active worksheet
...
def procedure_inventory_excel(request):
    ...
    #엑셀 파일 불러오기
    wb = load_workbook('static/excel_form/Procedure_list.xlsx') #Workbook()
    ws = wb.active# grab the active worksheet
...
def medicine_inventory_excel(request):
    ...
    #엑셀 파일 불러오기
    wb = load_workbook('static/excel_form/Medicine_list.xlsx') #Workbook()
    ws = wb.active# grab the active worksheet
...
def expendables_inventory_excel(request):
    ...
    #엑셀 파일 불러오기
    wb = load_workbook('static/excel_form/Expendable_list.xlsx') #Workbook()
    ws = wb.active# grab the active worksheet
...
def search_daily(request):
    ...
    for count in range(1,date_count + 1):
        ...
        paid = 0 if date_query['total_price'] is None else date_query['total_price']
        ...
        list_date_count.append({
            'id':2,
            'start':tmp_date.strftime('%Y-%m-%d'),
            'title':'Actual Amount : ' + str(0 if date_query['Actual_Amount'] is None else "{:,}".format(date_query['Actual_Amount'])) + " VND",
        ...
        list_date_count.append({
            'id':3,
            'start':tmp_date.strftime('%Y-%m-%d'),
            'title':'Debit payment : ' + str(0 if date_query['Debit_Payment'] is None else "{:,}".format(date_query['Debit_Payment'])) + " VND",
            'backgroundColor':'rgb(254,154,202)',
            'borderColor':'rgb(254,154,202)',
            })              
        list_date_count.append({
            'id':4,
            'start':tmp_date.strftime('%Y-%m-%d'),
            'title':'Unpaid : ' + str(0 if date_query['payment_total_unpaid'] is None else "{:,}".format(date_query['payment_total_unpaid'])) + " VND",
            'backgroundColor':'rgb(254,154,202)',
            'borderColor':'rgb(254,154,202)',
            })                 
        list_date_count.append({
            'id':5,
            'start':tmp_date.strftime('%Y-%m-%d'),
            'title':'Paid : ' + str(0 if date_query['total_price'] is None else "{:,}".format(date_query['total_price'])) + " VND",
            'backgroundColor':'rgb(254,154,202)',
            'borderColor':'rgb(254,154,202)',
            })
        list_date_count.append({
            'id':6,
            'start':tmp_date.strftime('%Y-%m-%d'),
            'title':'Due Amount : ' + str(0 if date_query['Due_Amount'] is None else "{:,}".format(date_query['Due_Amount'])) + " VND",
            'backgroundColor':'rgb(254,154,202)',
            'borderColor':'rgb(254,154,202)',
            })   
